...
print(f"板条梁仅受横荷重时的最大挠度为\t{w_max*1e3:.2f} mm")
...
print(f"T = {T:.4f} N")
sigma_0 = T / 1 / t
print(f"对应的单跨梁欧拉应力为\t{T/1/t*1e-6:.2f} MPa\n")

w_max_complex = c9m.midSpanDeflection2(q, l, c9m.E1(), t) * c9m.f0(u)
print(f"板条梁最大挠度为\t{w_max_complex*1e3:.2f} mm\n")

M_max = c9m.bendingMomentMidSpan2(q, l) * c9m.phai0(u)
print(f"板条梁最大弯矩为\t{M_max:.3f} N*m\n")
sigma_b_max = abs(c9m.bendingNormalStress(M_max, t))
print(f"板条梁最大弯曲正应力为\t{sigma_b_max*1e-6:.2f} MPa (拉应力)\n")
sigma_max = sigma_0 + sigma_b_max
print(f"板条梁最大应力为\t{sigma_max*1e-6:.2f} MPa (拉应力)\n")
