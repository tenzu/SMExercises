...
print(f"根据 u 计算板条梁中面力：")
T = 4 * u**2 * c9m.cylindricalStiffness(t) / l**2
print(f"T = {T:.4f} N")
sigma_0 = T / 1 / t
print(f"sigma_0 = {sigma_0 :4f} Pa = {T/1/t*1e-6:.2f} MPa\n")

print(f"计算板条梁最大挠度：")
w_max_complex = c9m.midSpanDeflection2(q, l, c9m.E1(), t)*c9m.f0(u)
print(f"w_max_complex = {w_max_complex:.4f} m = {w_max_complex*1e3:.2f} mm\n")
...
print(f"sigma_b_max = {sigma_b_max:.3f} Pa = {sigma_b_max*1e-6:.2f} MPa (拉应力)")
print(f"计算板条梁最大应力：")
sigma_max = sigma_0+sigma_b_max
print(f"sigma_max = {sigma_max:.3f} Pa = {sigma_max*1e-6:.2f} MPa (拉应力)\n")
