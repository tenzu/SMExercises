...
print(f"w_A = {w_A :.4f} m = {w_A*1e3 :.3f} mm\n")
...
print(
    f"sigma_A_complex = {sigma_0+sigma_A_complex :.2f} Pa = {(sigma_0+sigma_A_complex)*1e-6 :.2f} MPa\n"
)

print(f"计算A点复杂弯曲挠度：")
print(f"f1(u) = {c9m.f1(u) :.3f}")
w_A_complex = w_A * c9m.f1(u)
print(f"w_A_complex = {w_A_complex :.4f} m = {w_A_complex*1e3 :.3f} mm\n")
...
print(
    f"sigma_B_complex = {sigma_0+sigma_B_complex :.2f} Pa = {(sigma_0+sigma_B_complex)*1e-6 :.2f} MPa\n"
)
