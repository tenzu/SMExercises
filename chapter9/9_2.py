...
print(f"板条梁复杂弯曲时的跨中挠度为\t{w_max_complex * 1e3 :.3f} mm\n")

M_A = c9m.bendingMomentMidSpan2(q, l) * c9m.phai0(u)
print(f"A点弯矩为\t{M_A :.4f} N*m\n")
sigma_A_top = sigma_0 - abs(c9m.bendingNormalStress(M_A, t))
print(f"A点上表面正应力为\t{sigma_A_top * 1e-6 :.3f} MPa (拉应力)\n")
sigma_A_bottom = sigma_0 + abs(c9m.bendingNormalStress(M_A, t))
print(f"A点下表面正应力为\t{sigma_A_bottom * 1e-6 :.3f} MPa (拉应力)\n")
